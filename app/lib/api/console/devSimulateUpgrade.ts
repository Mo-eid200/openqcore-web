// app/lib/api/console/devSimulateUpgrade.ts
//
// ⚠️ DEV-ONLY — calls the temporary /dev/simulate/upgrade and
// /dev/simulate/renewal backend endpoints, scoped to context_type='user'
// ONLY. Delete this file once real 2Checkout credentials are wired up
// via createCheckout() from lib/api/console/billing.ts.
//
// 🔒 By design, this file's functions take NO contextType parameter —
// they always pass "user" to the backend. This makes it structurally
// impossible to call this from console/billing/page.tsx with a
// workspace id by mistake; the workspace equivalent lives in its own
// file (lib/api/workspace/devSimulateUpgrade.ts) which is hardcoded
// to "workspace" the same way. Each page imports only the file that
// matches its own context.
...
export async function simulatePersonalUpgrade(
  userId: string,
  planId: number,
...
export async function simulatePersonalRenewal(
  userId: string
