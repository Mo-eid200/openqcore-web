...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "foundation_community_page.seo",
  });
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      url: "https://openqcore.com/foundation/community",
      siteName: "OpenQCore",
    },
    alternates: {
      canonical: "https://openqcore.com/foundation/community",
      languages: {
        en: "https://openqcore.com/en/foundation/community",
        ar: "https://openqcore.com/ar/foundation/community",
      },
    },
  };
}
...
export default async function FoundationCommunityPage({
  params,
}: PageProps) {
  const { locale } = await params;
  const t = await getTranslations({
    locale,
    namespace: "foundation_community_page",
  });

  const whyCards = ["learn", "collaborate", "build"] as const;
    ...
    <main className="min-h-screen bg-[#050911] text-white">
          ...
          <div className="grid items-center gap-10 lg:grid-cols-12 lg:gap-12">
            <div className="lg:col-span-7">
              ...
              <div className="mt-10 flex flex-wrap gap-4">
                <PrimaryCTA href="/company/contact?intent=community">
                  {t("hero.cta_primary")}
                </PrimaryCTA>

                <SecondaryCTA href="/foundation">
            ...
            <div className="lg:col-span-5">
              <div className="relative ms-auto w-full max-w-[560px]">
                ...
                <div className="relative rounded-[26px] border border-white/[0.08] bg-[#0b1222]/85 p-2.5 shadow-[0_20px_70px_rgba(0,0,0,0.28)]">
                  <ImageWithLightbox
                    src="/engines/openqcore-community-hero.png"
                    alt="OpenQCore Community"
                    width={1400}
                    height={1000}
      ...
      <section className="border-b border-white/[0.05]">
        <div className={`${wrap} py-24 md:py-28`}>
          ...
          <p className="mt-4 max-w-3xl text-slate-300">{t("why.subtitle")}</p>
        ...
        <div className={`${wrap} py-24 md:py-28`}>
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {initiatives.map((k) => (
              <article key={k} className={card}>
                <h3 className="text-lg font-bold text-white">
                  {t(`initiatives.cards.${k}.title`)}
                </h3>
                <p className="mt-3 text-sm leading-7 text-slate-300">
                  {t(`initiatives.cards.${k}.desc`)}
                </p>
              </article>
            ))}
          </div>
      ...
      <section className="border-b border-white/[0.05]">
        <div className={`${wrap} py-24 md:py-28`}>
          <SectionLabel>{t("future.title")}</SectionLabel>
