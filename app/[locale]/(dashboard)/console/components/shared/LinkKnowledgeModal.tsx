...
import { getKnowledgeItems, getKnowledgeAgents, linkKnowledgeToAgent, unlinkKnowledgeFromAgent } from "@/app/lib/api/console/knowledge";
import { getAgents, getAgentKnowledgeLinks, linkAgentToKnowledge, unlinkAgentFromKnowledge } from "@/app/lib/api/console/agents";
...
export function LinkKnowledgeModal({
  ...
  const { data: knowledgeData, isLoading: loadingKnowledge } = useQuery({
    queryKey: ["console-knowledge-all"],
    queryFn:  () => getKnowledgeItems(200, 0),
    enabled:  open && isAgentMode,
    staleTime: 30_000,
  });

  const { data: agentsData, isLoading: loadingAgents } = useQuery({
    queryKey: ["console-agents-all"],
    queryFn:  () => getAgents(200, 0),
    enabled:  open && !isAgentMode,
    staleTime: 30_000,
  });
  ...
  const { data: linkedKnowledgeIds, isLoading: loadingLinkedForAgent } = useQuery({
    queryKey: ["console-agent-knowledge-links", entityId],
    queryFn:  () => getAgentKnowledgeLinks(entityId),
    enabled:  open && isAgentMode,
    staleTime: 10_000,
  });

  const { data: linkedAgents, isLoading: loadingLinkedForKnowledge } = useQuery({
    queryKey: ["console-knowledge-agent-links", entityId],
    queryFn:  () => getKnowledgeAgents(entityId),
    enabled:  open && !isAgentMode,
    staleTime: 10_000,
  });
  ...
  async function toggle(targetId: string, currentlyLinked: boolean) {
    ...
    try {
      if (isAgentMode) {
        if (currentlyLinked) {
          await unlinkAgentFromKnowledge(entityId, targetId);
        } else {
          await linkAgentToKnowledge(entityId, targetId);
        }
        queryClient.invalidateQueries({ queryKey: ["console-agent-knowledge-links", entityId] });
      } else {
        if (currentlyLinked) {
          await unlinkKnowledgeFromAgent(entityId, targetId);
        } else {
          await linkKnowledgeToAgent(entityId, targetId);
        }
        queryClient.invalidateQueries({ queryKey: ["console-knowledge-agent-links", entityId] });
      }
    } finally {
      setPendingId(null);
    }
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/[0.05] bg-amber-300/[0.10] text-amber-300">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-2 text-center">
              <p className="text-xs text-white/40">
                {isAgentMode ? "No knowledge sources yet" : "No agents yet"}
              </p>
            </div>
          ) : (
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${linked ? "bg-amber-300/[0.08] border border-amber-300/20" : "border border-transparent hover:bg-white/[0.04]"}
                      disabled:opacity-50
                    `}
                  >
                    <div className={`
                      flex h-5 w-5 shrink-0 items-center justify-center rounded-md border
                      ${linked ? "bg-amber-300 border-amber-300" : "border-white/[0.15] bg-white/[0.02]"}
                    `}>
                      {pending ? (
