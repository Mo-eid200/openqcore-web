...
import { getKnowledgeItems } from "@/app/lib/api/console/knowledge";
import {
...
export function KnowledgeTab({ agentId, agentName }: { agentId: string; agentName: string }) {
  ...
  const { data: linkedIds, isLoading: loadingLinks } = useQuery({
    queryKey: ["console-agent-knowledge-links", agentId],
    queryFn: () => getAgentKnowledgeLinks(agentId),
    staleTime: 10_000,
  });
  ...
  async function toggle(itemId: string, linked: boolean) {
    ...
    try {
      if (linked) {
        await unlinkAgentFromKnowledge(agentId, itemId);
      } else {
        await linkAgentToKnowledge(agentId, itemId);
      }
      queryClient.invalidateQueries({ queryKey: ["console-agent-knowledge-links", agentId] });
    } finally {
      setPendingId(null);
    }
  }
    ...
    <div className="rounded-2xl border border-white/[0.06] bg-[#0f1012]/92 p-5 backdrop-blur-xl">
      ...
      {isLoading ? (
        ...
        <p className="py-8 text-center text-xs text-white/40">
          No knowledge sources uploaded yet. Go to the Knowledge page to add some.
        </p>
      ) : (
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isLinked ? "bg-amber-300/[0.08] border border-amber-300/20" : "border border-transparent hover:bg-white/[0.04]"}
                  disabled:opacity-50
                `}
              >
                <div className={`
                  flex h-5 w-5 shrink-0 items-center justify-center rounded-md border
                  ${isLinked ? "bg-amber-300 border-amber-300" : "border-white/[0.15] bg-white/[0.02]"}
                `}>
                  {pending ? (
