...
type Props = {
    ...
    /**
     * Fallback model id used whenever selectedModel is not provided —
     * e.g. on the marketing/landing page where ChatFooter is used standalone
     * with `defaultModelId="quark"`. Keep this in sync with whatever default
     * you pass to <ChatFooter defaultModelId="..." /> so voice and text
     * requests always hit the same model.
     */
...
export const useVoice = ({
    ...
    // resolved model id used for every backend call in this hook —
    // single source of truth so meta + audio requests never diverge.
    const resolvedModelId = selectedModel?.id || defaultModelId;
    ...
    const fetchMeta = useCallback(
        async (blob: Blob, turnId: string): Promise<MetaResponse> => {
            ...
            formData.append("model", resolvedModelId);
        ...
        [API_BASE, abortWithTimeout, buildAuthHeaders, onMessageAction, resolvedModelId, sessionId]
    );
    ...
    const fetchAudio = useCallback(
        ...
        [API_BASE, abortWithTimeout, buildAuthHeaders, playAudioResponse, resolvedModelId, sessionId]
    );
